...
Foretoken 是一个面向 SLO/SLA 与异构硬件的大模型推理编排框架。

Foretoken 基于 vLLM、SGLang 等推理引擎，把多个推理实例组织成一套集群服务，负责请求路由、自动扩缩容、实例管理和性能评测。
...
- 需要跨硬件友好，需要支持 NVIDIA、沐曦、昇腾等不同硬件上使用同一套编排方案。

如果只在单张卡上运行一个模型，直接使用推理引擎通常就够了。
...
| 分布式推理 | 聚合部署、Prefill/Decode 分离和WideEP并行策略 | 研究中 |
...
`main` 分支目前还没有可运行组件。第一个基线合并后，这里会提供部署命令、示例请求和结果检查方法。
