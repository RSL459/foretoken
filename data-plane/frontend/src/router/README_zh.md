...
| Scorer | `kv_least_loaded`、`least_loaded`、`uniform`、`running_request` | `kv_least_loaded` | 为保留目标评分 |
...
将 `scorer` 设为 `running_request`，即可优先选择引擎调度器中运行的请求较少的目标。

该策略只使用 Model Server 端点的当前指标，不叠加前缀位置、待派发请求或下游阶段负载。
同一 Model Server 的所有 DP rank 共享端点评分，无法通过该策略区分各 rank 的负载。
收到首个遥测响应后即可使用 gauge，无需等满速率统计窗口。从未上报的指标按零处理；
后续响应缺失某项指标时，保留该项之前的值。
