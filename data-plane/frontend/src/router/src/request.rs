...
pub struct RouterRequest {
    ...
    /// Route target previously selected for this conversation session, resolved by the caller.
...
impl RouterRequest {
    ...
    /// Sets the route target previously selected for this conversation session.
    ...
    /// Returns the stable request identity used for exact pending-dispatch ownership.
    ...
    /// Returns the route target previously selected for this session, when supplied by the caller.
    pub fn session_target_id(&self) -> Option<&str> {
        self.session_target_id.as_ref().map(RouteTargetId::as_str)
    }

    /// Returns the LoRA adapter referenced by this request, when present.
    ...
    /// Returns the prompt plus declared maximum output used by token-load routing.
    pub fn estimated_total_tokens(&self) -> u64 {
        u64::try_from(self.prompt_token_ids().len())
            .unwrap_or(u64::MAX)
            .saturating_add(u64::from(self.max_new_tokens()))
    }
