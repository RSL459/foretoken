...
KV 前缀索引像一份缓存目录：它记录“哪个模型实例保存了哪些 prompt 前缀”。KV cache 本身仍由推理后端保存，索引只记录查询路由所需的信息。

## 在 Foretoken 中的位置

KV 前缀索引位于推理后端和 Router 之间：
...
- KV 前缀索引维护每个路由目标和 DP rank 的缓存目录。
- Router 通过 `KvPrefixIndexer` 查询缓存命中情况，再结合负载等信息选择路由。

索引不负责保存或传输 KV cache，也不替 Router 做路由决策。
...
不同事件源、路由目标和 DP rank 的记录相互隔离，不会混用不同模型实例的数据。
